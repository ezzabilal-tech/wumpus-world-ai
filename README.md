...
AI Logic Tech Course University
...
Adds ¬L (negation of query) to a working copy of the KB
Iteratively applies the resolution rule: from (A ∨ B) and (¬B ∨ C) derive (A ∨ C)
If an empty clause (contradiction) is derived → L is proven true
If no new clauses can be generated → L cannot be proven from current knowledge
...
🕹️ How to Use
Set grid size — Rows (3–8), Columns (3–8), Pit Count
Click ⬡ Initialize World to generate a random environment
Choose speed: Slow / Med / Fast / Turbo
Click ▶ Step Once to advance one decision at a time, or ⚡ Run Auto to watch the agent navigate autonomously
Use 👁 Reveal All to see the true world layout (ground truth)
Monitor the right panel for:
